...
export default function Java() {
  const [code, setCode] = useState("// Type Java code here");
  ...
  const handleJavainputChange = (e) => {
    setInput(e.target.value);
  };

  const compileCodess = async () => {
    const requestData = {
      code: code,
      input: input,
    };
    ...
    try {
      const fetchdata = await axios.post(
        "http://localhost:4000/compile/java",
        requestData
      );
      ...
      console.log("error", e);
      setOutput("Error occur during comilation");
    }
  };
    ...
    <>
        ...
        <div className="col-span-1 bg-[#000000c2] rounded">
          <div className="flex flex-col p-[8px] rounded">
            ...
            onChange={handleJavainputChange}
          ...
          <div className="flex justify-center items-center mt-[12px] mb-[8px]">
            <button onClick={compileCodess} className="site_btn px-[60px]">Run</button>
          </div>
        </div>
